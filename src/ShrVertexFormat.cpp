...
SHR_IMPLEMENT_RTTI(shr, Object, VertexFormat);
SHR_IMPLEMENT_STREAM(VertexFormat);
SHR_IMPLEMENT_FACTORY(VertexFormat);
SHR_IMPLEMENT_DEFAULT_NAMES(Object, VertexFormat);
...
void VertexFormat::Load (InStream& source)
{
    ...
    SHR_END_DEBUG_STREAM_LOAD(VertexFormat, source);
}
//----------------------------------------------------------------------------
...
void VertexFormat::Save (OutStream& target) const
{
    ...
    SHR_END_DEBUG_STREAM_SAVE(VertexFormat, target);
}
//----------------------------------------------------------------------------
int VertexFormat::GetStreamingSize () const
{
    ...
    for (int i = 0; i < AM_MAX_ATTRIBUTES; ++i)
    {
        ...
        size += SHR_ENUMSIZE(mElements[i].Type);
        size += SHR_ENUMSIZE(mElements[i].Usage);
