...
SHR_IMPLEMENT_RTTI(shr, Spatial, Node);
SHR_IMPLEMENT_STREAM(Node);
SHR_IMPLEMENT_FACTORY(Node);
...
Object* Node::GetObjectByName (const std::string& name)
{
    ...
    {
        SHR_GET_OBJECT_BY_NAME(mChild[i], name, found);
    }
    ...
    std::vector<Object*>& objects)
{
    ...
    for (int i = 0; i < numChildren; ++i)
    {
        SHR_GET_ALL_OBJECTS_BY_NAME(mChild[i], name, objects);
    }
}
//----------------------------------------------------------------------------
...
void Node::Load (InStream& source)
{
    ...
    SHR_END_DEBUG_STREAM_LOAD(Node, source);
}
//----------------------------------------------------------------------------
...
void Node::Save (OutStream& target) const
{
    ...
    SHR_END_DEBUG_STREAM_SAVE(Node, target);
}
//----------------------------------------------------------------------------
int Node::GetStreamingSize () const
{
    ...
    size += numChildren*SHR_POINTERSIZE(mChild[0]);
    return size;
}
//----------------------------------------------------------------------------
