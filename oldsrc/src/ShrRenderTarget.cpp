...
SHR_IMPLEMENT_RTTI(shr, Object, RenderTarget);
SHR_IMPLEMENT_STREAM(RenderTarget);
SHR_IMPLEMENT_FACTORY(RenderTarget);
...
Object* RenderTarget::GetObjectByName (const std::string& name)
{
    ...
    {
        SHR_GET_OBJECT_BY_NAME(mColorTextures[i], name, found);
    }
    SHR_GET_OBJECT_BY_NAME(mDepthStencilTexture, name, found);
    ...
    std::vector<Object*>& objects)
{
    ...
    for (int i = 0; i < mNumTargets; ++i)
    {
        SHR_GET_ALL_OBJECTS_BY_NAME(mColorTextures[i], name, objects);
    }
    SHR_GET_ALL_OBJECTS_BY_NAME(mDepthStencilTexture, name, objects);
}
//----------------------------------------------------------------------------
...
void RenderTarget::Load (InStream& source)
{
    ...
    SHR_END_DEBUG_STREAM_LOAD(RenderTarget, source);
}
//----------------------------------------------------------------------------
...
void RenderTarget::Save (OutStream& target) const
{
    ...
    SHR_END_DEBUG_STREAM_SAVE(RenderTarget, target);
}
//----------------------------------------------------------------------------
int RenderTarget::GetStreamingSize () const
{
    int size = Object::GetStreamingSize();
    size += mNumTargets*SHR_POINTERSIZE(mColorTextures[0]);
    size += SHR_POINTERSIZE(mDepthStencilTexture);
    size += SHR_BOOLSIZE(mHasMipmaps);
    return size;
}
//----------------------------------------------------------------------------
