...
SHR_IMPLEMENT_RTTI(shr, Object, Shader);
SHR_IMPLEMENT_STREAM(Shader);
SHR_IMPLEMENT_ABSTRACT_FACTORY(Shader);
SHR_IMPLEMENT_DEFAULT_NAMES(Object, Shader);
...
void Shader::Load (InStream& source)
{
...
#ifdef SHR_ASSERT_ON_CHANGED_MAX_PROFILES
    ...
    SHR_END_DEBUG_STREAM_LOAD(Shader, source);
}
//----------------------------------------------------------------------------
...
void Shader::Save (OutStream& target) const
{
    ...
    SHR_END_DEBUG_STREAM_SAVE(Shader, target);
}
//----------------------------------------------------------------------------
int Shader::GetStreamingSize () const
{
    ...
    {
        size += SHR_STRINGSIZE(mInputName[i]);
        size += SHR_ENUMSIZE(mInputType[i]);
        size += SHR_ENUMSIZE(mInputSemantic[i]);
    }

    size += sizeof(mNumOutputs);
    for (i = 0; i < mNumOutputs; ++i)
    {
        size += SHR_STRINGSIZE(mOutputName[i]);
        size += SHR_ENUMSIZE(mOutputType[i]);
        size += SHR_ENUMSIZE(mOutputSemantic[i]);
    }

    size += sizeof(mNumConstants);
    for (i = 0; i < mNumConstants; ++i)
    {
        size += SHR_STRINGSIZE(mConstantName[i]);
    ...
    for (i = 0; i < mNumSamplers; ++i)
    {
        size += SHR_STRINGSIZE(mSamplerName[i]);
        size += SHR_ENUMSIZE(mSamplerType[i]);
        size += SHR_ENUMSIZE(mFilter[i]);
        size += SHR_ENUMSIZE(mCoordinate[0][i]);
        size += SHR_ENUMSIZE(mCoordinate[1][i]);
        size += SHR_ENUMSIZE(mCoordinate[2][i]);
    ...
    size += SHR_BOOLSIZE(mProfileOwner);
    if (mProfileOwner)
    {
        for (i = 0; i < MAX_PROFILES; ++i)
        {
            ...
            size += SHR_STRINGSIZE(*mProgram[i]);
        }
    }
