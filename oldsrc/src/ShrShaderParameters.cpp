...
SHR_IMPLEMENT_RTTI(shr, Object, ShaderParameters);
SHR_IMPLEMENT_STREAM(ShaderParameters);
SHR_IMPLEMENT_FACTORY(ShaderParameters);
...
Object* ShaderParameters::GetObjectByName (const std::string& name)
{
    ...
    SHR_GET_OBJECT_BY_NAME(mShader, name, found);

    int i;
    for (i = 0; i < mNumConstants; ++i)
    {
        SHR_GET_OBJECT_BY_NAME(mConstants[i], name, found);
    }

    for (i = 0; i < mNumTextures; ++i)
    {
        SHR_GET_OBJECT_BY_NAME(mTextures[i], name, found);
    }
    ...
    std::vector<Object*>& objects)
{
    ...
    SHR_GET_ALL_OBJECTS_BY_NAME(mShader, name, objects);

    int i;
    for (i = 0; i < mNumConstants; ++i)
    {
        SHR_GET_ALL_OBJECTS_BY_NAME(mConstants[i], name, objects);
    }

    for (i = 0; i < mNumTextures; ++i)
    {
        SHR_GET_ALL_OBJECTS_BY_NAME(mTextures[i], name, objects);
    }
}
//----------------------------------------------------------------------------
...
void ShaderParameters::Load (InStream& source)
{
    ...
    SHR_END_DEBUG_STREAM_LOAD(ShaderParameters, source);
}
//----------------------------------------------------------------------------
...
void ShaderParameters::Save (OutStream& target) const
{
    ...
    SHR_END_DEBUG_STREAM_SAVE(ShaderParameters, target);
}
//----------------------------------------------------------------------------
int ShaderParameters::GetStreamingSize () const
{
    int size = Object::GetStreamingSize();
    size += SHR_POINTERSIZE(mShader);
    size += sizeof(mNumConstants);
    size += mNumConstants*SHR_POINTERSIZE(mConstants[0]);
    size += sizeof(mNumTextures);
    size += mNumTextures*SHR_POINTERSIZE(mTextures[0]);
    return size;
}
//----------------------------------------------------------------------------
