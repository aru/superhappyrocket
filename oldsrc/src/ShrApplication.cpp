...
int main (int numArguments, char* arguments[])
{
    ...
    // Memory class in LibCore/Memory/Memory.h for the signatures of these
    // functions.
    Memory::Initialize();
#endif
    ...
    // PATH environment variable in order for the applications to find
    // various data files.
#ifndef __APPLE__
    Application::Path = Environment::GetVariable("PATH");
#else
    ...
    if (system("cp ~/.MacOSX/path.txt temppath.txt") == 0)
    {
        std::ifstream inFile("temppath.txt");
        if (inFile)
        {
            getline(inFile, Application::Path);
            inFile.close();
            system("rm temppath.txt");
        }
    }
#endif
    if (Application::Path == "")
    {
        assertion(false, "Please set the PATH environment variable.\n");
        std::ofstream outFile("ApplicationError.txt");
        if (outFile)
        {
            outFile << "Please set the PATH environment variable.\n";
            outFile.close();
        }
        return INT_MAX;
    }
    ...
    // using BIG_ENDIAN/LITTLE_ENDIAN.  The folder names for
    ...
    directory = Application::Path + std::string("Data/Wmfx/");
    Environment::InsertDirectory(directory);

    // The path to scene object files.
    directory = Application::Path + std::string("Data/Wmof/");
    Environment::InsertDirectory(directory);

    // The path to texture image files.
    directory = Application::Path + std::string("Data/Wmtf/");
    Environment::InsertDirectory(directory);

    // The path to vertex/index buffer files.
    directory = Application::Path + std::string("Data/Wmvf/");
    Environment::InsertDirectory(directory);

    // The path to image files.
    directory = Application::Path + std::string("Data/Im/");
    Environment::InsertDirectory(directory);
