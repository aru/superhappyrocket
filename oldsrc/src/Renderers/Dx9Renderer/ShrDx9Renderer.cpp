...
#ifdef SHR_RESET_STATE_AFTER_DRAW
...
#define SHR_BEGIN_QUERY(device, query) \
    IDirect3DQuery9* query = BeginQuery(device)

#define SHR_END_QUERY(query, numPixelsDrawn) \
...
void Renderer::DrawPrimitive (const Visual* visual)
{
    ...
    ||  type == Visual::PT_TRIFAN)
    {
        ...
        {
            SHR_BEGIN_QUERY(mData->mDevice, query);
            ...
            SHR_END_QUERY(query, numPixelsDrawn);
        }
    }
